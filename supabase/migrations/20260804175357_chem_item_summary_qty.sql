-- Per-item comparisons read in QUANTITY (tabs, pounds, quarts) — more
-- intuitive than dollars and the same signal since the unit price is
-- constant. Dollars stay for the group/category rollups where mixed
-- units can't sum.
...
RETURNS TABLE(item_name text, this_qty numeric, self_avg_qty numeric, peer_avg_qty numeric, this_usd numeric, self_avg_usd numeric, peer_avg_usd numeric)
...
AS $$
  ...
  SELECT s.item_name,
         COALESCE(sum(s.qty) FILTER (WHERE s.customer_id = p_customer_id AND s.month = p_month), 0),
         COALESCE(sum(s.qty) FILTER (WHERE s.customer_id = p_customer_id AND s.month < p_month), 0)
           / GREATEST((SELECT self_months FROM denom), 1),
         COALESCE(sum(s.qty) FILTER (WHERE s.customer_id <> p_customer_id), 0)
