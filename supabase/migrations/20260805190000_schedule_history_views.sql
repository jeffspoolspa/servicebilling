...
-- Not every task fact carries a uuid: batch remediations name themselves
-- (e.g. 'batch:pre-refresh-2-remediation'), so ::uuid blows this up on real
-- data. The id stays TEXT; task_id is a null-safe cast for joining.
DROP VIEW IF EXISTS maintenance.v_schedule_history;
DROP VIEW IF EXISTS maintenance.v_schedule_changes_open;

CREATE VIEW maintenance.v_schedule_history AS
SELECT
  e.seq, e.occurred_at,
  e.aggregate_id AS task_ref,
  CASE WHEN e.aggregate_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
       THEN e.aggregate_id::uuid END AS task_id,
  e.type, e.actor,
...
CREATE VIEW maintenance.v_schedule_changes_open AS
WITH req AS (
...
SELECT r.aggregate_id AS task_ref, r.occurred_at AS requested_at,
       ...
       round((EXTRACT(epoch FROM (now() - r.occurred_at))/60)::numeric, 1) AS minutes_open
FROM req r
