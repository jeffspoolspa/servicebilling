-- The stamp fn writes BOTH links: the invoice, and the exact QBO LINE when
-- the match is unambiguous (invoice + item-name leaf + rate + parsed break
-- date on itemized docs). Ambiguous stays NULL — honest. Supersedes
-- 20260804150401's single-stamp version.
    ...
    SELECT bi.id AS item_id,
           (SELECT il.qbo_line_id FROM billing.invoice_lines il
            WHERE il.qbo_invoice_id = bi.qbo_invoice_id
              ...
              AND upper(split_part(COALESCE(il.item_name, ''), ':', array_length(string_to_array(COALESCE(il.item_name,''), ':'), 1))) = upper(bi.item_name)
