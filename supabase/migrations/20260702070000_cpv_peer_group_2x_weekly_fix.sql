-- Peer-group fix: 2x-weekly RESIDENTIAL customers belong in weekly_residential, not commercial.
-- The v1 billing-audit skill design: weekly_residential = weekly + 2x_weekly residential; only
-- HIGH-frequency (3x+/week) residential joins the commercial peer group. The visits-based view's
-- "any multi_week task -> commercial" was too aggressive (caught LEICHERT/CAIN/SAMS/WAITES et al at
-- 2 days/week). Rule now: commercial = company filled (QBO rule) OR any recurring task with
-- days_per_week > 2.
...
select
  ...
  case when nullif(trim(coalesce(c.company,'')),'') is not null or v.high_freq then 'commercial'
