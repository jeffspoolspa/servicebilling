-- maint_billing_periods: add office (Customers.office_id -> branches.name)
-- and segment (company-filled => commercial, else residential — QBO is the
-- source; see project_task_classification) for the Bills table tabs.
-- Return shape changes -> DROP + CREATE. Everything else identical to
-- 20260702180000.
...
returns table (
  ...
  office                    text,
  segment                   text      -- commercial | residential
)
