...
import { HOME_OFFICES, listRouteLoad, listRouteStopsAll } from "../../_lib/route-analysis"
...
export default async function TerritoryMapPage() {
  ...
  const anomalies = stops.filter((s) => s.far_from_route)
  ...
  const mapStops: OverviewStop[] = located.map((s) => {
    const sub = [
      ...
      s.city ?? null,
      s.far_from_route
        ? `${Math.round(s.route_center_mi ?? 0)} mi from route center — check address`
  ...
  const anomalyRows = [...anomalies].sort((a, b) => (b.route_center_mi ?? 0) - (a.route_center_mi ?? 0))
  ...
  return (
    <div className="px-7 pt-6 pb-10 space-y-6">
      <div className="flex items-end justify-between">
        <div>
          ...
          <div className="text-ink-mute text-[12px] mt-1">
            Every active stop with a rooftop-confirmed location, colored by office. Red-ringed stops sit far from
            the rest of their route — usually a wrong service address.
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <Metric value={String(anomalies.length)} label="far from route (check address)" tone={anomalies.length ? "coral" : undefined} />
      ...
      <Card>
        <div className="px-5 py-3.5 border-b border-line-soft flex items-center justify-between">
          <h3 className="font-display text-[15px]">Far from their route — likely wrong address</h3>
          <span className="text-[11px] text-ink-mute">{anomalyRows.length} stops to check</span>
        </div>
        <table className="w-full text-[12px]">
          <thead>
            <tr className="text-left text-ink-mute border-b border-line-soft">
              <th className="px-4 py-2 font-medium">Customer</th>
              <th className="px-4 py-2 font-medium">Service address</th>
              <th className="px-4 py-2 font-medium">Route</th>
              <th className="px-4 py-2 font-medium text-right">Mi from route</th>
              <th className="px-4 py-2 font-medium text-right">Nearest pool</th>
            </tr>
          </thead>
          <tbody>
            {anomalyRows.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-ink-mute">
                  No stops sit far from their route.
                </td>
              </tr>
            )}
            {anomalyRows.map((s) => (
              <tr key={s.schedule_id} className="border-b border-line-soft/40 last:border-0 hover:bg-white/[0.02]">
                ...
                <td className="px-4 py-2.5 text-right font-mono num text-coral">
                  {s.route_center_mi != null ? Math.round(s.route_center_mi) : "—"}
                </td>
                <td className="px-4 py-2.5 text-right font-mono num text-ink-mute">
                  {s.nearest_mate_mi != null ? Math.round(s.nearest_mate_mi) : "—"}
                </td>
              </tr>
            ))}
          </tbody>
