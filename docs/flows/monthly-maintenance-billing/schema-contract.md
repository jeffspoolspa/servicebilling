...
- `public.maint_billing_*` RPCs (migration `20260702130000`) — the app-facing read surface for the `/maintenance/billing` UI (billing_audit is not PostgREST-exposed): `maint_billing_months` (month rollups), `maint_billing_periods` (promises joined with customer, `maintenance.v_task_class`, ION transaction sums, the `billing.invoices` mirror, and a DERIVED processing status: pending → held_for_review | ready → processed → paid), `maint_billing_review_flags` (the 2x-median queue), `maint_billing_flags` + `maint_billing_flag_items` (CPV z-audit review + per-item drill-down), `maint_billing_flag_review` (the one write: audit_status/audit_notes). SECURITY DEFINER, authenticated-only.
