...
5. **Preprocess (queued, serial)** — `drain_maint_preprocess_queue` every 2 min runs `preprocess_maint_customer_month` one at a time: customer-scoped credit apply (no email) → projection evaluates chem_flag (2x rule) / ION amount / subtotal (ION vs QBO total) / reconcile verdict — gates evaluate ONLY at preprocess → `needs_review` | `ready_to_process`.
