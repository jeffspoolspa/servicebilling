...
3. **ION match** — `match_promises_to_ion`: stamp each promise with its ION invoice number + amount from `ion_task_transactions` → `ion_matched` (stamp only — no gate fires before preprocess). Runs (a) right after the UI's **Refresh bills** button pulls the ION report (`f/ION/transactions_report` — the deliberate MANUAL trigger, usable mid-month; no schedule) and (b) on the hourly reconcile as a backstop.
...
5. **Preprocess (queued, serial)** — `drain_maint_preprocess_queue` every 2 min runs `preprocess_maint_customer_month` one at a time: customer-scoped credit apply (no email) → projection evaluates chem_flag (2x rule) / ION amount / subtotal (ION vs QBO SUBTOTAL — pre-tax, pre-discount; tax and QBO-side discounts never trip it) / reconcile verdict — gates evaluate ONLY at preprocess → `needs_review` | `ready_to_process`.
...
7. **Charge/send (queued, drained; 2026-07-10)** — the `ready_to_process` transition enqueues the customer-month onto `billing_audit.maint_charge_queue` (`trg_enqueue_maint_charge`, coalesced). The human-kicked worker `f/billing/process_maint_charges` ([conventions/WORKFLOW_EXECUTION.md](../../conventions/WORKFLOW_EXECUTION.md)) drains it until empty: claim-time resolve → no autopay = send invoices; autopay = `charge_and_record` (ADR 009: one charge for the summed fresh balances, one QBO payment across the month's invoices, receipt first, invoice copies after; WAL + idempotent resume inside the service). The worker stamps no status — `processed` derives via `project_maint_processing_status` (paid+sent echoes, the attempt log, or delivered-without-charge for email-path/declined months). Superseded: `process_maint_period` [retired pending verification], the roster-driven [monthly-autopay](../monthly-autopay.md) [retired].
