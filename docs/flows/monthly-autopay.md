...
5. **build_list** — join `maintenance_invoices` to [autopay_customers](../entities/autopay-customer.md), pull ALL unpaid maintenance invoices (current + prior months), pre-create [autopay_transactions](../entities/autopay-transaction.md) with a UNIQUE (customer, billing_month) constraint so a re-run can't double-charge. **Excludes any invoice-month whose customer has an unreviewed HIGH flag in `billing_audit.customer_month_audit`** (the billing-audit hold; reviewed via `/maintenance/billing/review`) and reports the held count as `held_high_flag_customers`. Optional `only_qbo_customer_ids` filter for batch selection from `/maintenance/billing/process`. (Both are in the repo mirror; **deploy pending** — the live flow predates them.)
