...
| ConsumablesAudit | domain service | **[built]** the pre-invoice billing check: chem-per-visit vs the peer group's percentile (peer = `billing_audit.v_customer_peer_group`: commercial / high_freq_residential / weekly_residential / low_freq — the same ruled axis the live chem-flag medians use) AND the customer's own trailing median (both bars must be crossed). Repository selects the criteria rows (`chemHistory`, `taskPeerKeys`), `observationsOf` is the factory, the domain judges. Findings -> `billing.findings`, which is what the gate's `findings_resolved` criterion holds on — audit writes, gate holds. Policy (percentile, min peers, self factor, min history, bulk-allowed groups) is an explicit `AUDIT_POLICY` object. Chem provision is a TASK attribute overriding the customer's demographic group: `customer_provides_chems` tasks form the `provides_chems` peer group (rule: ANY chem billing above a tolerance is a `chems_billed_to_provider` finding, no percentile); `bulk_refill` tasks form the `bulk_refill` group (bucket deliveries expected). Bulk containers (`maintenance.consumables.is_bulk`, a catalog fact) are excluded from every CPV number, and a bulk item on any task NOT marked bulk_refill is an unconditional `bulk_item_misbill` finding — the tech keyed the bucket instead of the single-unit item; the finding tells the reviewer to mark the task if the delivery is genuine. READ MODEL + REVIEW LOOP: `billing.v_findings_review` feeds /maintenance/billing/findings (Findings tab); marking a finding reviewed writes resolved_at/by/resolution (mandatory reason), a resolved finding never resurrects, and the month is re-enqueued at interactive priority so the next advance re-gates it. |
