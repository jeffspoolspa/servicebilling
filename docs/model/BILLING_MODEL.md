...
RULED PIPELINE (Carter 2026-08-03, superseding the earlier month-grained preprocess/process):
THE MONTH'S OWN LIFECYCLE ENDS AT INVOICE CREATION. accrue -> reconcile -> gate -> ISSUE
(documentsOf -> QboInvoices.createInvoice; enrichment folded into creation) — and that is the
month's last own step (nextStep after invoiced = null). THE HANDOFF: each created invoice enters
