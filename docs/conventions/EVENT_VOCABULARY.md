...
| `MonthItemsExcluded` / `MonthItemsRestored` | MonthItemsExcluded / MonthItemsRestored | authored — a person, per item or per task-month (RULED 2026-08-07) | billable items marked NON-BILLABLE (or restored): they stay on the ledger — the work happened, reconcile still counts them against ION — but documents skip them; they never reach an invoice. The mark survives re-accrual (claim preserves the decision). Frozen once invoiced. payload `{count, task_id?, item_ids?}` |
| `MonthServiceEnded` | MonthServiceEnded | authored — a person, via the month page "Issue early" (RULED 2026-08-07) | the billing period follows the SERVICE: a cancellation closes it early, making the month billable now (the calendar month-end is just the default close). Not a gate bypass — endService un-reconciles, so the ladder re-runs reconcile (fresh-basis trust window unchanged) and a fresh gate before issue. Idempotent: the first fact stands. payload `{reason, endedAt}` |
