...
- **Data path is pure HTTP.** chromium is needed **only for login** (the Imperva JS challenge in front of ionpoolcare.com; the Fluidra portal hop it also used to clear was retired by ION in 2026-08). After that, the entire prime + fetch runs over raw `ionFetch` with the captured cookies — verified by pulling the 487-row recurring-tasks report (200, byte-identical to the manual download) with **no browser navigation** after login.
